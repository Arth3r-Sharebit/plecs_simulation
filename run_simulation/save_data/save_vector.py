...
CH_NAMES = [
    ...
    "S1_Pcond",
    "S1_Psw",
    "S1_Ptot",  # Port2: S1 Loss
    "S2_Pcond",
    "S2_Psw",
    "S2_Ptot",  # Port3: S2 Loss
    "S3_Pcond",
    "S3_Psw",
    "S3_Ptot",  # Port4: S3 Loss
    "S4_Pcond",
    "S4_Psw",
    "S4_Ptot",  # Port5: S4 Loss
    "Tj_S1",
    "Tj_S2",
    "Tj_S3",
    "Tj_S4",
...
def save_data_and_plot(sim_results, freq, duty, rcs, rsa, csa, vin, rl, k, folder, path: str | None = None):
    ...
    temp_data = {
        CH_NAMES[15 + i]: v_full[15 + i, ::step_temp]
        for i in range(5)
        if (15 + i) < n_ch
    }
    ...
    df_ts = pd.DataFrame(
        {"Time": t_temp, **{ts: temp_data[n] for ts, n in zip(ts_names, temp_names)}}
    )
...
def save_data_and_plot_json(sim_results, freq, duty, rcs, rsa, csa, vin, rl, k, folder, path: str | None = None):
    ...
    v_full = np.array(
        sim_results["result"]["Values"]
    )  # JSON中Values已经是(n_ch, n_samples)格式
    n_ch = v_full.shape[0]
